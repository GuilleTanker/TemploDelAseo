...
public class ConsultasFacturas extends javax.swing.JInternalFrame {
     ...
     void cargartodasfacturas()
    {
        ...
        String []titulos={"Numero","Subtotal","Total","Fecha","Cod. Cliente","Forma De Pago" };
        tabla.setColumnIdentifiers(titulos);
        this.tbfacturas.setModel(tabla);
        String consulta= "SELECT * FROM facturaespecial ";
        ...
        try {
            ...
            while(rs.next())
            {  
                Datos[0]=rs.getString("idFactEspecial");
                Datos[1]=rs.getString("subtotal");
                Datos[2]=rs.getString("total");
                Datos[3]=rs.getString("fec_fac");
                Datos[4]=rs.getString("cliente_cod_cli");
                Datos[5]=rs.getString("formadepago");
    ...
    private void initComponents() {
        ...
        btnbuscador.setIcon(new javax.swing.ImageIcon(getClass().getResource("/com/img/Search (2).png"))); // NOI18N
...
// TODO add your handling code here:
    ...
    {
        consulta= "SELECT * FROM facturaespecial WHERE idFactEspecial='"+num+"'";
    }
...
String fec=""+formatofecha.format(fecha);
        consulta= "SELECT * FROM facturaespecial WHERE fec_fac='"+fec+"'";
    }
    if(rdbntodos.isSelected()==true)
    {
        consulta= "SELECT * FROM facturaespecial ";
    }
        DefaultTableModel tabla= new DefaultTableModel();
        String []titulos={"Numero","Subtotal","Total","Fecha","Cod. Cliente","Forma De Pago" };
        tabla.setColumnIdentifiers(titulos);
...
// TODO add your handling code here:
    ...
    detalle.setVisible(true);
        ...
        String subtotal=tbfacturas.getValueAt(filasele, 1).toString();
        String total=tbfacturas.getValueAt(filasele, 2).toString();
        String fecha=tbfacturas.getValueAt(filasele, 3).toString();
        String cod=tbfacturas.getValueAt(filasele, 4).toString();
...
//        DetalleFactura.txtruc.setText(ruc);
        ...
        String ver="SELECT * FROM detallefactura WHERE FacturaEspecialidFactEspecial='"+numfac+"'";
        String []datos= new String[6]   ;
        try {
                Statement st = cn.createStatement();
                ...
                while(rs.next())
                {
                    datos[0]=rs.getString("des_pro");
                    datos[1]=rs.getString("cant_pro");
                    datos[2]=rs.getString("pre_unit");
                    datos[3]=rs.getString("pre_tot");
                    datos[4]=rs.getString("ProductoEspecialIdProductoEspecial");
                    datos[5]=rs.getString("FacturaEspecialidFactEspecial");
                    model.addRow(datos);
...
// TODO add your handling code here:
    ...
    if(fila>=0)
    {
        String cod=tbfacturas.getValueAt(fila, 0).toString();
            try {
                PreparedStatement pst = cn.prepareStatement("DELETE FROM facturaespecial WHERE idfacturaespecial='"+cod+"'");
                pst.executeUpdate();
            } catch (SQLException ex) {
